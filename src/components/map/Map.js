...
class Map extends Component {
  ...
  getUserResults(e, map){
    console.log(e);
    console.log(map);
    this.map.getSource('home').setData(e.result.geometry);
    ...
    .then(function(data) {
      ...
      map.getSource('poll').setData(point.geometry);
      ...
      ${data.features.map(feature => `
        <div class="item">
          ...
          `<strong>DISTRICT:</strong> ${feature.properties.district_l}<br>
          <strong>ELECTED OFFICIAL:</strong> ${feature.properties.representa}<br>` : `<strong>LOCATION:</strong> ${feature.properties.precinct_n}<br>
          <strong>ADDRESS:</strong> ${feature.properties.precinct_l}<br>`}
        </div>
        `
      ).join('')}
      `;
